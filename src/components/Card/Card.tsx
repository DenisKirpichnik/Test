import React from "react";
import { JOIBtn, JOIdiv, JOItext } from "../common/SharedStyles";
import { BottomFlexDiv, CardHeaderDiv, MainCardWrapper, TopFlexDiv } from "./CardStyles";
import styled from "styled-components";
import { useHistory } from "react-router-dom";

const SvgBinanceIcon = () => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      width="61"
      height="61"
      viewBox="0 0 61 61"
    >
      <defs>
        <pattern id="pattern" preserveAspectRatio="none" width="100%" height="100%" viewBox="0 0 200 200">
          <image
            width="200"
            height="200"
            xlinkHref="data:image/png;base64,"
          />
        </pattern>
      </defs>
      <rect id="_1839" data-name="1839" width="61" height="61" rx="30.5" fill="url(#pattern)" />
    </svg>
  );
};

interface Props {
  projectName: string;
}

export const Card: React.FC<Props> = ({ projectName }) => {
  const history = useHistory();

  const handleClick = () => {
    history.push("/project");
  };

  return (
    <MainCardWrapper
      descr="MAIN CARD WRAPPER"
      flexdirection="column"
      background="white"
      width="443px"
      height="100%"
      borderradius="10px"
      boxshadow="0px 1px 1px 1px rgba(0, 0, 0, 0.2)"
      padding="0px 0px 50px 0px"
    >
      <TopFlexDiv>
        <JOIdiv
          flexdirection="row"
          background="white"
          margin="10px 15px 0px auto"
          width="90%"
          color="#c265e3"
          justifycontent="space-between"
        >
          <JOItext margin="15px auto 0px 10px" color="#c265e3" fontsize="35px">
            {projectName}
          </JOItext>
          <SvgBinanceIcon />
        </JOIdiv>

        <JOItext color="#c265e3" margin="20px auto 0px 37px" fontweight="400" width="300px">
          Stake BNB, earn JOI
        </JOItext>
      </TopFlexDiv>
      <JOIdiv
        descr="BOTTOM FLEX DIV"
        flexdirection="column"
        background="white"
        margin="5px 0px 0px 0px"
        padding="5px 0px 50px 0px"
      >
        <JOIdiv
          flexdirection="column"
          background="white"
          margin="10px 15px 0px auto"
          width="90%"
          color="#c265e3"
          justifycontent="space-between"
        >
          <JOItext color="#c265e3" fontweight="400" fontsize="25px">
            simply dummy text of the printing and typesetting industry. industry's standard dummy...{" "}
          </JOItext>
          <JOItext color="#c265e3" fontweight="600" fontsize="25px">
            More info
          </JOItext>
        </JOIdiv>
        <JOIdiv
          flexdirection="row"
          background="white"
          margin="30px auto 0px auto"
          width="68%"
          color="#c265e3"
          justifycontent="space-between"
        >
          <JOItext color="#c265e3" fontweight="600" fontsize="25px" margin="10px 0px 0px 0px">
            APR
          </JOItext>
          <JOItext color="#c265e3" fontweight="600" fontsize="25px" margin="10px 0px 0px 0px" width="60px">
            365%
          </JOItext>
        </JOIdiv>

        <JOIBtn
          width="243px"
          fontweight="bold"
          margin="30px auto 0px auto"
          onClick={() => handleClick()}
          background="rgba(194,101,227,1)"
          padding="5px 0px 5px 0px"
        >
          Stake
        </JOIBtn>
      </JOIdiv>
    </MainCardWrapper>
  );
};
